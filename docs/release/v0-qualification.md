...
build, tarball inspection, and corrected read-only Habit Compass rerun are locally exercised, but
release readiness is not claimed. Supported Node CI, the full fresh-install qualification, an
independent specification completion review, and separate approval of the concrete Habit Compass
proposal remain completion gates.
...
| 12  | Production/tar inspection passes; the harness denies source-checkout reads, resolves the installed adapter inside the temporary install, and loads both packaged skills, but local installation remains certificate-blocked. | partial     |
